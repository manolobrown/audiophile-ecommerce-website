...
const isHome = computed(() => {
  return route.name === "index" ? "bg-[#191919]" : "bg-black";
});
</script>
<template>
  <div class="navbar-wrapper z-10 relative" :class="isHome">
    ...
    >
      ...
      <ul class="hidden navbar__nav xl:flex flex-row gap-[34px]">
        <li v-for="link in navLinks">
          <NuxtLink
            :to="link.url"
            class="text-white font-bold uppercase text-[13px] leading-[25px] tracking-[2px] transition-colors hover:text-[#D87D4A]"
            >{{ link.text }}</NuxtLink
          >
          >
        </li>
